...
public class PlateState : IPlateState
{
    ...
    public void UpdateFoodsOnPlate(List<int> _allChilds)
    {
        for(int i=0; i<_allChilds.Count; i++)
        {
            ...
            else if (_allChilds.Contains(1) && _allChilds.Contains(3) && _allChilds.Contains(4) && _allChilds.Contains(5))
                _foodsOrder = FoodsOrder.MeatCheeseTomato;
            else if (_allChilds.Contains(1) && _allChilds.Contains(3) && _allChilds.Contains(4))
                _foodsOrder = FoodsOrder.MeatCheese;
            else if (_allChilds.Contains(1) && _allChilds.Contains(4))
